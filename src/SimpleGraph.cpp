...
void InitGraphVisualizer(SimpleGraph &userGraph) {
  ...
  QObject::connect(&userGraph, SIGNAL(graphUpdated(SimpleGraph)), &g,
                   SLOT(repaint()));
}
...
void MyWidget::paintEvent(QPaintEvent *event) {
  ...
  if (!userGraph.nodes.empty()) {
    ...
    double maxX =
        std::max_element(copy.nodes.begin(), copy.nodes.end(), getX)->x;
    double minX =
        std::min_element(copy.nodes.begin(), copy.nodes.end(), getX)->x;

    auto getY = [](const Node &a, const Node &b) { return a.y < b.y; };
    double maxY =
        std::max_element(copy.nodes.begin(), copy.nodes.end(), getY)->y;
    double minY =
        std::min_element(copy.nodes.begin(), copy.nodes.end(), getY)->y;
    ...
    auto scale = [maxX, maxY, minX, minY](const Node &a) -> Node {
      return {(a.x - minX) * (kWindowWidth - kCircleDiameter) / (-minX + maxX) +
                  kCircleRadius,
              (a.y - minY) * (kWindowHeight - kCircleDiameter) /
                      (-minY + maxY) +
                  kCircleRadius};
    };

    std::transform(copy.nodes.begin(), copy.nodes.end(), copy.nodes.begin(),
                   scale);
    ...
    for (Node &n : copy.nodes) {
      painter.drawEllipse(n.x - kCircleRadius, n.y - kCircleRadius,
...
int main(int argc, char **argv) {
  ...
  myWidget.show();
  qRegisterMetaType<SimpleGraph>(); // allows use of SimpleGraph in
                                    // signals/slots
  WorkerThread x;
  x.start();
  return app.exec();
...
void DrawGraph(SimpleGraph &userGraph) { userGraph.drawGraph(userGraph); }
